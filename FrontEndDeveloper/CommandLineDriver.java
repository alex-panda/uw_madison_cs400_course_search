...
public class CommandLineDriver {
  ...
  private static void printTree(Course course) {
    if (course.hasPrereqs()) {
      ...
      for (int i = 0; i < prereqs.length; i++) {
        if (prereqs[i] instanceof Course) {
          if (((Course) prereqs[i]).hasPrereqs())
            printTree((Course) prereqs[i]);
          System.out.println(((Course) prereqs[i]).getName());
        }
        System.out.println();
      }
    } else
  ...
  public static void main(String[] args) {
    ...
    UserHashMapInterface application = new UserHashMapInterface();
